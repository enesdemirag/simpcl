...
int main(int argc, char **argv)
{
    ...
    nh_private.param<std::string>("subscribed_topic", subscribed_topic, "/odometry");
    nh_private.param<std::string>("base_frame", base_frame, "world");
    nh_private.param<std::string>("child_frame", child_frame, "stereo_camera");
    nh_private.param<int>("loop_rate", loop_rate, 10);

    // Subscribe
    ros::Subscriber sub = n.subscribe<nav_msgs::Odometry>(subscribed_topic, 1, callback);
